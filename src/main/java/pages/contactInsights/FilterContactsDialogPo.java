...
public class FilterContactsDialogPo extends BasePo {
    ...
    private By selectedOptionByDropdownName(String dropdownName) {
        return By.xpath("//span[text()='" + dropdownName + "']/ancestor::div[contains(@class, 'fields')]/ng-select//span[contains(@class, 'ng-value-label')]");
    }
    ...
    public void selectOptionFromDropdownByOptionName(String optionName) throws Exception {
        ...
        optionTextList = optionTextList.stream()
                                       .map(StringUtils::getStringByTrim)
    ...
    public void selectCheckboxByStatusAndOptionName(String checkboxStatus, String optionName) throws Exception {
        ...
        if (index != -1) {
            ...
            if ((Checks.equals(checkboxStatus) && !isSelected) ||
                (Unchecks.equals(checkboxStatus) && isSelected)) {
                Action.clickByLocator(ElementUtils.getElementByLocator(dropdownPanelOption, index));
            }
        } else {
            throw new NoSuchElementException("Dropdown option not found: " + optionName);
    ...
    public void clickOnCloseIconInSelectedOptionByDropdownAndOptionName(String dropdownName, String optionName) throws Exception {
        ...
        int index = optionTextList.indexOf(optionName);
        if (index != -1) {
            Action.clickByLocator(ElementUtils.getElementByLocator(selectedOptionCloseIconByDropdownName(dropdownName), index));
        } else {
